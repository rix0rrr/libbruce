...
TEST_CASE("test findinternalkeyindex", "[nodes]")
{
    ...
    node->insert(2, node_branch(two_r, 0, 0));
    node->insert(3, node_branch(three_r, 0, 0));
    ...
    SECTION("find 2")
    {
        ...
        REQUIRE( FindInternalKey(node, two_r, intToIntTree) == 0 );
    }
    SECTION("find 5")
    {
        REQUIRE( FindInternalKey(node, intCopy(5), intToIntTree) == 3 );
    }
    SECTION("find 0")
    {
        // Completely to the left
