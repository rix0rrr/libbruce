...
TEST_CASE("seeking in a plain tree", "[query][rank]")
{
    ...
    tree<uint32_t, uint32_t>::iterator it;
    it = query.seek(0);
    REQUIRE(it.value() == 1);
...
TEST_CASE("queued upsert")
{
    ...
    put_result root = make_internal()
        ...
        .brn(make_leaf(intToIntTree)
           ...
           .put(mem)) // 1
        .put(mem); // 2
    tree<int, int> query(root.nodeID, mem);

    SECTION("upsert becomes an update")
    {
        query.upsert(1, 2, false);
